...
class Webpages():
    ...
    @staticmethod
    def build_banned_page(banned_until):
        print("BANNED PAGE", banned_until)
        banned_page = """
        <!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
  ...
  <script>
  CountDownTimer(""" + str(banned_until) + ", " + str(int(time.time()) * 1000) + """ , 'countdown');
  // CountDownTimer('02/20/2012 10:1 AM', 'newcountdown');

    function CountDownTimer(dt, now_time, id)
    {
        console.error(dt)
        console.error(now_time)
        var end = new Date(dt);
        ...
        function showRemaining() {
          console.log("show remaining")
            ...
            var time_remaining = " for ";

            if (hours > 0) {
              time_remaining += hours + " hrs ";
            }
            if (minutes > 0) {
            time_remaining += minutes + " miutes and ";
            }
